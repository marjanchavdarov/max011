...
def upload():
    ...
    jobs[job_id] = {"status": "starting", "page": 0, "total_pages": 0, "products": 0, "log": [], "done": False, "error": None}
    ...
    def process():
        try:
            ...
            jobs[job_id]["log"].append("Started: " + str(total_pages) + " pages")
            ...
            for i in range(total_pages):
                try:
                    ...
                    jobs[job_id]["page"] = i + 1
                    jobs[job_id]["products"] = total_products
                    jobs[job_id]["log"].append("Page " + str(i+1) + "/" + str(total_pages) + ": " + str(len(products)) + " products found, " + str(saved) + " saved")
                except Exception as e:
                    jobs[job_id]["log"].append("Page " + str(i+1) + " error: " + str(e))
                    continue
            ...
            jobs[job_id]["log"].append("DONE! " + str(total_products) + " products saved!")
...
def save_products(products, store, page_num, page_url, catalogue_name, valid_from, valid_until):
    ...
    for p in products:
        ...
        if not vu:
            continue
        records.append({"store": store, "product": p.get("product", ""), "brand": p.get("brand") if p.get("brand") not in [None, "null"] else None, "quantity": p.get("quantity") if p.get("quantity") not in [None, "null"] else None, "original_price": p.get("original_price") if p.get("original_price") not in [None, "null"] else None, "sale_price": p.get("sale_price", ""), "discount_percent": p.get("discount_percent") if p.get("discount_percent") not in [None, "null"] else None, "category": p.get("category", "Ostalo"), "subcategory": p.get("subcategory"), "valid_from": valid_from, "valid_until": vu, "is_expired": False, "page_image_url": page_url, "page_number": page_num, "catalogue_name": catalogue_name, "catalogue_week": datetime.now().strftime("%Y-W%V")})
    if not records:
        return 0
