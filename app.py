...
def upload():
    ...
    fd = f.read() if f else None
    fn = f.filename if f else ""
    def stream():
        try:
            if not fd or not sn or not vf:
            ...
            for i in range(total_pages):
                try:
                    ...
                    saved = save_products(products, sn, i+1, page_url, cat_name, vf, valid_until)
            ...
            doc.close()
            os.remove(tmp)
            save_catalogue(sn, cat_name, vf, valid_until, cat_fp, total_pages, total_products)
